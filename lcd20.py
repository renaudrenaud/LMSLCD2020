    ...
    def main_loop(self):
        ...
        while True:
            ...
            if player_info is not None and player_info['isplaying'] ==1 and type(server_status) is dict:
                ...
                if player is not None and player["mixer volume"] != mixer_volume or change_volume is True:
                    if mixer_volume == 0:
                ...
                if int(song["id"]) != 0:
                    ...
                    if (song_info is None or song["id"] != song_info["songinfo_loop"][0]["id"]) or int(song["id"]) < 0:
                        ...
                        if song != last_song:
                            ...
                            if len(self.get_from_loop(song_info["songinfo_loop"], "remote_title")) > 0:
                                current_title = self.get_from_loop(song_info["songinfo_loop"], "remote_title") + "-" + current_title
                ...
                if self.display_mode == "volume" and player["time"] > 15 or change_volume == True:
                ...
                if type(server_status) is dict:
                    ...
                    if players is not None:
                        if len(players) > 0:
                            self.lcd.lcd_display_string("Players count: " + str(len(players)), 4)
                        else:
